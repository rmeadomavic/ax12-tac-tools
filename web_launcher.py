...
fetch('/api/setup-check').then(r=>r.json()).then(d=>{
  ...
  else{s2.textContent='NOT FOUND';s2.className='step-status fail';b2.innerHTML='<b>IMPORTANT:</b> The AX12 runs Android 9. ATAK 5.x requires Android 10+. You need <b>ATAK-CIV 4.10.x</b>.<br><br>Download from:<br>&bull; <a href="https://tak.gov/products" style="color:#6c6">tak.gov/products</a> (official, free registration)<br>&bull; <a href="https://www.civtak.org/download-atak/" style="color:#6c6">civtak.org/download-atak</a><br><br>Get the <b>4.10</b> APK. No Play Services needed — sideload directly.<br><br><button class="step-btn" onclick="location.reload()">CHECK AGAIN</button>'}
